// js/views/wa.js — F-06: daftar pesanan WA dikelompokkan. Lihat docs/01-PRD.md §3.2.
import { daftarPesananWa, konfirmasiPelangganWa } from '../api.js';
import { rupiah, jamId, labelHari } from '../format.js';
import { el, toast } from '../ui.js';
import { mulaiPesananBaru } from './wa-pesanan.js';

export async function render(container) {
  container.innerHTML =
    '<div style="padding:var(--s5);text-align:center;color:var(--abu-kabut);">Memuat pesanan WA...</div>';
  try {
    ...
    container.appendChild(
      el('div', { style: 'padding:var(--s5);text-align:center;color:var(--sambal);' }, [
        el('div', {}, 'Gagal memuat pesanan WA. Cek sinyal.'),
...
async function gambarUlang(container) {
  ...
  if (semua.length === 0) {
    bungkus.appendChild(
      el('div', { style: 'padding:var(--s6) 0;text-align:center;color:var(--abu-kabut);' },
        'Belum ada pesanan WA. Ketuk + untuk buat yang pertama.')
    );
  }
  ...
  const seksi = (judul, daftar) => {
    ...
    daftar.forEach((p) => bungkus.appendChild(kartuWa(p, container)));
  };
  ...
  const fab = el('button', { class: 'wa-fab', 'aria-label': 'Pesanan Baru' }, '+');
  fab.addEventListener('click', () => mulaiPesananBaru(container, () => render(container)));
  container.appendChild(fab);
}

function kartuWa(p, container) {
